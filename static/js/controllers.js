...
conferenceApp.controllers.directive('fileModel', ['$parse', function ($parse){
    return{
        restrict: 'A',
...
conferenceApp.controllers.controller('CreateReservationCtrl',
    function ($scope, $http, $log, oauth2Provider, HTTP_ERRORS) {
        ...
        $scope.isValidDates = function(){
            ...
            if (!$scope.reservation.startDate && !$scope.reservation.endDate) {
                return true;
            }
            if ($scope.reservation.startDate && !$scope.reservation.endDate) {
                return true;
            }
            return $scope.reservation.startDate <= $scope.reservation.endDate;
        }

        $scope.isValidReservation = function (reservationForm) {
        ...
        $scope.createReservation = function (reservationForm) {
            ...
            if (!$scope.isValidReservation(reservationForm)) {
                return;
            }
            console.log("Crear!!!");
            ...
            console.log($scope);
            gapi.client.conference.createReservation($scope.reservation).
                execute(function (resp) {
                    $scope.$apply(function () {
                        $scope.loading = false;
                        if (resp.error) {
                            ...
                            // The request has succeeded.
                            console.log('The conference has been created')
                            $scope.messages = 'The conference has been created : ' + resp.result.name;
                            $scope.alertStatus = 'success';
                            $scope.submitted = false;
                            $scope.conference = {};
