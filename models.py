...
class Reservation(ndb.Model):
    """Conference -- Conference object"""
    name            = ndb.StringProperty(required=True)
    depositId       = ndb.StringProperty()
    urlDepositId    = ndb.StringProperty()
    organizerUserId = ndb.StringProperty()
    startDate       = ndb.DateProperty()
    endDate         = ndb.DateProperty()
    month           = ndb.IntegerProperty()
...
class ReservationForm(messages.Message):
    """ConferenceForm -- Conference outbound form message"""
    name            = messages.StringField(1)
    depositId       = messages.StringField(2)
    urlDepositId    = messages.StringField(3)
    organizerUserId = messages.StringField(4)
    startDate       = messages.StringField(5)
    endDate         = messages.StringField(6)
    month           = messages.IntegerField(7, variant=messages.Variant.INT32)
